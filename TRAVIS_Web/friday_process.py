...
from friday_constants import (BASE, BASE_DATABASE, BASE_METADATA_DATABASE,
                              BASE_METADATA_TABLE, BASE_TABLE,
                              COMPARE_REPORT_FILE, CSV, EXCEPTION_FILE_NAME,
                              IMAGE_PREFIX, JSON, MATCH, MATCH_FILE_NAME,
                              MESSAGE_LOOKUP, MISMATCH, OUT_OF_SEQ_FILE_NAME,
                              RELEASE, RELEASE_DATABASE,
                              RELEASE_METADATA_DATABASE,
                              RELEASE_METADATA_TABLE, RELEASE_TABLE,
                              TAGGING_REPORT_FILE, UNMATCH_FILE_NAME)
from travis_exceptions import ProcessingException, ValidationException
...
class CompareFiles(ABC):
    ...
    def _validate_output_store_base_release_details(self) -> None:
        ...
        if self.output_store_base_release is None:
            self.message = MESSAGE_LOOKUP.get(12) %("Base Release Data Store")
            raise ValidationException(self.message)
    ...
    def __get_mismatch_summary_from_sqlite(self) -> 'list[tuple]':
        ...
        database_object = CompareDatabase(os.path.join(self.output_location, "mismatch_data.db"))
    ...
    def __get_mismatch_data_from_sqlite(self, limit=50) -> 'list[tuple]':
    ...
    def __consolidate_unmatch_files(generate_summary, output_code_page, output_delimiter, retain_temp, output_location, unmatch_file_name, output_header) -> None:
        ...
        database_object = CompareDatabase(os.path.join(output_location, "mismatch_data.db"))
    ...
    def __consolidate_match_files(generate_summary, output_code_page, output_delimiter, retain_temp, output_location, match_file_name, output_header) -> None:
        ...
        database_object = CompareDatabase(os.path.join(output_location, "match_data.db"))
    ...
    def _create_metadata_database(self, output_location, file_type="Base") -> None:
        ...
        database_object = CompareDatabase(os.path.join(output_location, f"{file_type.lower()}_metadata.db"))
    ...
    def _compare_base_release_metadb(self, output_location, db_key = "FILE_INDEX") -> None:
        ...
        database_object = CompareDatabase(os.path.join(output_location, BASE_METADATA_DATABASE))
        ...
        base_release_rows = database_object.run_attach_sql((os.path.join(output_location, RELEASE_METADATA_DATABASE),), 
    ...
    def _compare_base_not_in_release_metadb(self, output_location, db_key="FILE_INDEX") -> None:
        ...
        database_object = CompareDatabase(os.path.join(output_location, BASE_METADATA_DATABASE))
        base_not_in_release_rows = database_object.run_attach_sql((os.path.join(output_location, RELEASE_METADATA_DATABASE),), 
    ...
    def _compare_release_not_in_base_metadb(self, output_location, db_key="FILE_INDEX") -> None:
        ...
        database_object = CompareDatabase(os.path.join(output_location, BASE_METADATA_DATABASE))
        release_not_in_base_rows = database_object.run_attach_sql((os.path.join(output_location, RELEASE_METADATA_DATABASE),), 
    ...
    def _create_data_database(self, output_location, file_type="Base") -> None:
        ...
        database_object = CompareDatabase(os.path.join(output_location, f"{file_type.lower()}_data.db"))
    ...
    def _get_common_base_release_count(output_location) -> None:
        ...
        database_object = CompareDatabase(os.path.join(output_location, BASE_DATABASE))
        common_key_count = database_object.run_attach_sql((os.path.join(output_location, RELEASE_DATABASE),), 
                                                          attach_sql)
    ...
    def _get_release_data_count(output_location) -> None:
        ...
        # create sqlite connection to db
        database_object = CompareDatabase(os.path.join(output_location, RELEASE_DATABASE))
    ...
    def _get_compare_data_sublist(output_location, limit, base_rid, release_rid) -> None:
        ...
        # create sqlite connection to db
        database_object = CompareDatabase(os.path.join(output_location, BASE_DATABASE))
        sub_list_data = database_object.run_attach_sql((os.path.join(output_location, RELEASE_DATABASE),), 
                                                          attach_sql)
    ...
    def _get_base_not_in_release(output_location):
        ...
        base_not_in_release_rows = database_object.run_attach_sql((os.path.join(output_location, RELEASE_DATABASE), ), 
                                                                  sql)
    ...
    def _get_release_not_in_base(output_location):
        ...
        release_not_in_base_rows = database_object.run_attach_sql((os.path.join(output_location, RELEASE_DATABASE), ), 
                                                                  sql)
    ...
    def _load_file_metadata(self, file_dict, output_location, db_name=BASE_METADATA_DATABASE, thread_name="Thread-z", table_name=BASE_METADATA_TABLE, output_code_page="utf-8", output_delimiter=",") -> None:
        ...
        database_object = CompareDatabase(os.path.join(output_location, db_name))
...
class CompareMetaData(CompareFiles):
    ...
    def validate_file_keys(self) -> bool:
        ...
        if self.compare_file_keys.lower() not in ["file_name", "file_index"]:
            self.message = MESSAGE_LOOKUP.get(12) %("Metadata Compare File Keys")
    ...
    def get_base_release_metadata(self) -> None:
        ...
        base_tasks = self.get_file_metadata(
            self.base_file_sublist, BASE_METADATA_DATABASE, BASE_METADATA_TABLE)
        
        release_tasks = self.get_file_metadata(
            self.release_file_sublist, RELEASE_METADATA_DATABASE, RELEASE_METADATA_TABLE)
...
class JsonDynamicCompare(CompareFiles):
    ...
    def get_base_release_pdf_page_details(self):
        """ Get pdf details for base and release files """

        mylogger.info(friday_reusable.get_function_name())
        self.put_status_message_queue(status="Getting PDF Metadata")

        base_data = {}
        release_data = {} 

        # create file dictionary 
        for index, file_dict in enumerate(self.base_file_sublist):

            for file, file_index in file_dict.items():
                file_open = PdfReader(file)
                base_data[file_index] = [os.path.basename(file), file_open.pages, ]

        # create base and release tasks
        base_tasks = self.get_pdf_file_data(
            self.base_file_sublist, BASE_DATABASE, BASE_TABLE)
        
        release_tasks = self.get_pdf_file_data(
            self.release_file_sublist, RELEASE_DATABASE, RELEASE_TABLE)
...
class PDFFileCompare(CompareFiles):
    def __init__(self, config: dict = {}, root_option: str = "", sub_option: str = "", mypath: str = "", template_location: str = "", deloitte_image: str = "", travis_image: str = "", application_name: str = "", environment_name: str = "", run_id: int = 1, travis_status_queue: queue.Queue = None, treeview=None, open_browser: bool = True, merge_match_unmatch: bool = False) -> None:
        super().__init__(config, root_option, sub_option, mypath, template_location, deloitte_image, travis_image, application_name, environment_name, run_id, travis_status_queue, treeview, open_browser, merge_match_unmatch)

        self.compare_level = None
    ...
    def compare_pdf_files(self):
        ...
        try:
            ...
            # get page details for PDF files and store in JSON Database 
            self.get_base_release_pdf_page_details()

            # compare base and release json files 
            self.compare_batch_size = 100
            self.compare_skip_fields = []
            self.compare_case_sensitive = False
            self.perform_checksum_compare()

            # consolidate all temporary files 
            self._merge_temp_files()            

            # once compare is complete, prepare for text compare
            self._perform_text_compare()
    ...
    def validate_compare_level(self):
        """Validate compare configurations"""

        mylogger.info(friday_reusable.get_function_name())
        self.put_status_message_queue(status="Validating Release")

        self.compare_level = self.compare_config.get("Compare_Level", None)

        # file keys
        if self.compare_level is None:
            self.message = MESSAGE_LOOKUP.get(12) %("Compare Level")
            raise ValidationException(self.message)

        if len(self.compare_level) == 0:
            self.message = MESSAGE_LOOKUP.get(12) %("Compare Level")
            raise ValidationException(self.message)
    ...
    def get_pdf_file_data(self, file_sublist, db_name, table_name) -> list:
        """get file metadata"""

        mylogger.info(friday_reusable.get_function_name())
        self.put_status_message_queue(status="Getting PDF Data")
        ...
        for index, file_dict in enumerate(file_sublist):
            thread_name = "Thread-" + str(index)
            t = Thread(target=PDFFileCompare.load_pdf_file_data, args=(file_dict,
                                                                       self.compare_file_keys,
                                                                       self.output_location,
                                                                       db_name,
                                                                       thread_name,
    ...
    def load_pdf_file_data(file_dict, compare_file_keys, output_location, db_name=BASE_DATABASE, thread_name="Thread-z", table_name=BASE_TABLE, output_code_page="utf-8", output_delimiter=",") -> None:
        ...
        for file, file_index in file_dict.items():
            try:
                pdf_file_json = {} 
                pdf_file_json["File_Name"] = os.path.basename(file)
                
                # read the PDF file 
                file_open = PdfReader(file)
                pdf_file_json["Total_Pages"] = len(file_open.pages)

                # create list of dictionaries for each page
                for i in range(pdf_file_json["Total_Pages"]):
                    page = file_open.pages[i]
                    text = str(page.extract_text())
                    hash_code = hashlib.md5(bytes(text, "utf-8")).hexdigest()
                    pdf_file_json[f"Page_Number_{i+1}"] = hash_code

                # check the file key 
                if compare_file_keys == "File_Index":
                    base_tuple = (file_index, json.dumps(pdf_file_json, cls=CustomJSONEncoder), file, file_index)
                else: 
                    base_tuple = (os.path.basename(file), json.dumps(pdf_file_json, cls=CustomJSONEncoder), file, file_index)
    ...
    def perform_checksum_compare(self):
        """ perform checksum compare at page level for pdf files """
    ...
    def _perform_text_compare(self):
        """ get records from output mismatch table and get the corresspoding records """

        mylogger.info(friday_reusable.get_function_name())
        self.put_status_message_queue(status="Performing Text Compare")        
        
        # get file indexes which have errored out 
        mismatched_file_index = self._get_file_index_mismatch()
        
        # iterate and get file index and get the coresspoding page number causing issue 
        for row in mismatched_file_index:
            
            # create html difference object 
            html_difference = difflib.HtmlDiff()

            # get file index
            if self.compare_file_keys == "File_Index":
                file_index = int(row[0])
                
                # get the page number 
                mismatched_page_details = self._get_file_page_mismatch(file_index)

                # identify base and release file using index and create PDF object
                base_pdf_file = PdfReader(self.base_absolute_file_name[file_index])
                release_pdf_file = PdfReader(self.release_absolute_file_name[file_index])    

                # create base & release text file name 
                base_text_file_name = "PDF_Raw_Text_" + os.path.basename(self.base_absolute_file_name[file_index]).split(".")[0] + ".txt"
                release_text_file_name = "PDF_Raw_Text_" + os.path.basename(self.release_absolute_file_name[file_index]).split(".")[0] + ".txt"

                # create an output file 
                base_text_file = open(os.path.join(self.output_location, base_text_file_name), "w", encoding="utf-8")
                release_text_file = open(os.path.join(self.output_location, release_text_file_name), "w", encoding="utf-8")

                # iterate over page number and extract the text in a text file 
                for page_row in mismatched_page_details:
                    page_number = int(page_row[0].split("_")[-1])
                    page_index =  page_number - 1
                    
                    # get the base and release pages 
                    base_page = base_pdf_file.pages[page_index]
                    release_page = release_pdf_file.pages[page_index]

                    # write base data into a text file 
                    base_text = str(base_page.extract_text())
                    base_text_file.write(base_text)

                    # write release data into a text file 
                    release_text = str(release_page.extract_text())
                    release_text_file.write(release_text)

                    # write the output html file 
                    output_html_name = f"PDF_Compare_Text_File_Number_{file_index}_PageNumber_{page_number}.html"
                    ...
                    with open(os.path.join(self.output_location, output_html_name), "w", encoding="utf-8") as output_html:
                        html_diff = html_difference.make_file(base_text.splitlines(True), 
                                                            ...
                                                            self.base_absolute_file_name[file_index], 
                                                            self.release_absolute_file_name[file_index])
                ...
                file_name = row[0]
                base_file_index = int(self.base_file_name.index(file_name))
                release_file_index = int(self.release_file_name.index(file_name))

                file_index = base_file_index
                
                # get the page number 
                mismatched_page_details = self._get_file_page_mismatch("'" + file_name + "'")

                # identify base and release file using index and create PDF object
                base_pdf_file = PdfReader(self.base_absolute_file_name[base_file_index])
                release_pdf_file = PdfReader(self.release_absolute_file_name[release_file_index])    

                # create base & release text file name 
                base_text_file_name = "PDF_Raw_Text_Base_" + file_name.split(".")[0] + ".txt"
                release_text_file_name = "PDF_Raw_Text_Release_" + file_name.split(".")[0] + ".txt"
                ...
                for page_row in mismatched_page_details:
                    ...
                    # write the output html file 
                    output_html_name = f"PDF_Compare_Text_File_Number_{base_file_index}_PageNumber_{page_number}.html"

                    with open(os.path.join(self.output_location, output_html_name), "w", encoding="utf-8") as output_html:
                        html_diff = html_difference.make_file(base_text.splitlines(True), 
                                                            release_text.splitlines(True), 
                                                            self.base_absolute_file_name[base_file_index], 
                                                            self.release_absolute_file_name[release_file_index])
                        output_html.write(html_diff)
    ...
    def _get_file_index_mismatch(self) -> 'list[tuple]':
        ...
        # mismatch data SQL
        get_mismatch_summary_fields = """ SELECT DISTINCT KEY_ID
                                            FROM MISMATCH_TABLE ; """ 
    ...
    def _get_file_page_mismatch(self, file_index, field_name="Page_Number%") -> 'list[tuple]':

        database_object = CompareDatabase(os.path.join(self.output_location, "mismatch_data.db"))

        # get mismatch data at page level 
        get_mismatch_page_details = f""" SELECT FIELD_NAME 
                                         FROM MISMATCH_TABLE
                                         WHERE KEY_ID = {file_index} 
                                         AND FIELD_NAME LIKE '{field_name}' ;"""
        
        # mismatch data SQL 
        mismatch_table = database_object.get_rows(get_mismatch_page_details)
        database_object.disconnect()
...
class CsvStreamCompare(CompareFiles):
    ...
    def compare_csv_streams(self): 
        ...
        try:
            ...
            # Stream compare must not be used if files are unbalanced 
            if len(self.base_file_name) != len(self.release_file_name):
                self.message = MESSAGE_LOOKUP.get(15) %(CSV)
                raise ProcessingException(self.message)
...
class CsvDynamicCompare(CompareFiles):
    ...
    def compare_csv_data(self):
        ...
        self.message = MESSAGE_LOOKUP.get(14) %("CSV Dynamic Compare", self.output_location)
        self.put_status_message_queue(status="Completed", message=self.message)
