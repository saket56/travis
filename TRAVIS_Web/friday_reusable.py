...
def setup_user_workspace(workspace_directory:str="", workspace_name="Travis") -> 'tuple(str, dict)':
    ...
    if workspace_directory != "" and workspace_directory is not None:
        if os.path.exists(workspace_directory) and not os.path.isfile(workspace_directory): 
            # workspace_directory = os.path.join(workspace_directory, getpass.getuser(), workspace_name)
            workspace_directory = os.path.join(workspace_directory, workspace_name)
        elif os.path.exists(workspace_directory) and os.path.isfile(workspace_directory): 
            # workspace_directory = os.path.join(os.path.dirname(workspace_directory), getpass.getuser(), workspace_name)
            workspace_directory = os.path.join(os.path.dirname(workspace_directory), workspace_name)
        else:
            workspace_directory = os.path.join(os.path.expanduser("~/Documents"), workspace_name)
    else:
        workspace_directory = os.path.join(os.path.expanduser("~/Documents"), workspace_name)
...
def get_config_data(config_location:str="") -> 'dict':
    ...
    elif os.path.exists(os.path.join(os.path.dirname(config_location), "static", "data", "FridayConfig.yaml")):
        with open(os.path.join(os.path.dirname(config_location), "static", "data", "FridayConfig.yaml"), 'r') as configFile:
            config = yaml.safe_load(configFile)
            return config           
    else: 
