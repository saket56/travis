...
def create_user_workspace() -> 'tuple(str, dict)':
    ...
    # Cannot log as this is the first function called
    config = get_config_data()

    # get workpace location from the configuration 
    if 'FridayConfig' in config.keys(): 
        workspace_setting = config['FridayConfig'].get('workspace_setting')
        pwd = workspace_setting.get('workspace_dir') if 'workspace_dir' in workspace_setting.keys() else ""
    else: 
        message = "Invalid Configuration. Please check the FridayConfig.yaml file"
        raise ProcessingException(message)
       
    # Check if path exists
    if pwd != '' and os.path.exists(os.path.abspath(pwd)):
        pwd = os.path.join(os.path.abspath(pwd), getpass.getuser())
        os.mkdir(pwd) if not os.path.exists(pwd) else None
    else:
        pwd = os.path.expanduser('~/Documents')
    
    # create parent folder Friday if not present
    parent = os.path.join(pwd,'Travis')
    os.mkdir(parent) if not os.path.exists(parent) else None
    ...
    current_location = os.path.join(parent, workspace) 
    os.mkdir(current_location) if not os.path.exists(current_location) else None
    
    return current_location, config
...
def get_config_data() -> 'dict':
    ...
    if os.path.exists(os.path.join(pwd, "FridayConfig.yaml")):
        with open(os.path.join(pwd, 'FridayConfig.yaml'),'r') as configFile:
            config = yaml.safe_load(configFile)
            return config 

    elif os.path.exists(os.path.join(pwd, "static", "FridayConfig.yaml")):
        with open(os.path.join(pwd, "static", "FridayConfig.yaml"), 'r') as configFile:
            config = yaml.safe_load(configFile)
            return config             
    else: 
...
def purge_workspace_folders(current_location:'str' = "", parent_location:'str' = "", folder_list:'list'=[]) -> 'None':
    ...
    mylogger.info(get_function_name() + "\tParent Directory: " + str(parent_location))
...
def deduct_month(start_time:'datetime' = datetime.datetime.now(), number_of_month:'int'=0) -> 'datetime':
    ...
    date=start_time.date().replace(day=1) - datetime.timedelta(days=1)
...
def create_chunks(list_data:'list' = [], number_of_elements_in_chunk:int=1, number_of_chunks:int=1):
...
def merge_multiple_temp_files(output_location:'str'="", input_file_pattern:'str'="*.csv", output_file_name:'str'="", output_encoding:'str'="", first_record:str="") -> None:
...
def get_account_list(landing_zone = "", account_status = "", random_samples = 0, account_samples=[], account_root=""):
...
def perform_data_setup(output_location:str="", mypath:str="", sub_option:str="", application_name:str="", environment_name:str="", rename_existing=False) -> None:
    """ set up loot and output file location """ 
    
    mylogger.info(get_function_name())

    # if mypath is not populated 
    if output_location == "":
        ...
        output_file_location    = create_subfolder(output_location, 
                                                   mypath, 
                                                   str(application_name) + '_' + str(environment_name), 
                                                   rename_existing=True)        
...
def get_flat_json(data=None, header:list=[], separator:str=".", file_type:str="JSON") -> dict:
    ...
    if file_type.upper() == "CSV":
        ...
        sorted_data = perform_json_sorting(data)
        return flatten_json.flatten(OrderedDict(sorted_data), separator)



def perform_json_sorting(data):
    """ do data sorting at json level """

    if isinstance(data, dict):
        return sorted((key, perform_json_sorting(values)) for key, values in data.items() if values is not None) + [(key, values) for key, values in data.items() if values is None]
    
    # if data is list - sort the data in the list 
    elif isinstance(data, list):
        return sorted(perform_json_sorting(x) for x in data if x is not None) + [x for x in data if x is None]
