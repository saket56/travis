...
class App(ttk.Window,threading.Thread):
    ...
    def run(self):
        # self = tk.Tk()
            ...
            self.selectOperationDropDown['value']=['Batch Comparison','Individual Comparison','DB Connect','DDL Validation',
                                                  'TableStats','Utilities','Sqoop Batch Execution','PdfCompare', 'Line To Header file conversion']
            self.selectOperationDropDown.set('Operation Name')
            self.selectOperationDropDown.bind('<<ComboboxSelected>>', self.selectTestMethod)


            self.selected.set('Line To Header file conversion')
            ...
            self.frame_line_to_header=tk.LabelFrame(self.frame01,   borderwidth=10,text='Line to Header' )
            ...
            self.initiateUi_Line_to_Header()
    ...
    def initiateUi_Line_to_Header(self):
            print("in the UI function for line to header")
            tk.Label(self.frame_line_to_header, text="Line to Header Conversion",bg="white",fg="red", font=("Arial", self.fontSize+5)).grid(row=0,padx=(0,0),pady=15)
            
           
            self.line_to_header_template_btn=ttk.Button(
                self.frame_line_to_header,
                text=u'\u2193'+'Download Template',
                command=self.download_line_to_header_conversion_template,
                bootstyle="danger",
                # height= 1,
                  width=20,
                    # font=("Arial bold", self.fontSize),
            ...
            self.line_to_header_template_btn.grid(row=0,column=1,pady=5,padx=5,sticky='e')
            

            self.select_line_to_header_ExcelLocationBtn = ttk.Button(
            self.frame_line_to_header,
            text='Select Template Excel Path',
            command=self.startPoint_lth,
            # height= 1, 
            bootstyle="primary",
            width=25
            # font=("Arial", self.fontSize)
            ...
            self.select_line_to_header_ExcelLocationBtn.grid(row=1,column=0,pady=5,padx=50)
            
            
            self.lthExcelPath=ttk.Entry(self.frame_line_to_header, font="Arial 13")
            self.lthExcelPath.grid(row=1,column=1, padx= 50,pady=5,ipadx=200)

            self.select_line_to_header_SourceFolderBtn = ttk.Button(
            self.frame_line_to_header,
            text='Select Source Folder',
            command = self.select_line_to_header_Source_folder_location,
            ...
            self.select_line_to_header_SourceFolderBtn.grid(row=2,column=0,pady=5,padx=50)
            self.lth_Source_entry = ttk.Entry(self.frame_line_to_header, font="Arial 10 ")
            self.lth_Source_entry.grid(row=2,column=1,ipadx=220)
            ...
            # self.batch_release_entry = ttk.Entry(self.frame_line_to_header, font="Arial 10 ")
            ...
            self.selectlthResultFolderBtn = ttk.Button(
            self.frame_line_to_header,
            text='Select Result Folder',
            command=self.select_lth_Result_folder_location,
            # height= 1,
              width=25, 
              bootstyle="primary"
            #   font=("Arial", self.fontSize)
            ...
            self.selectlthResultFolderBtn.grid(row=4,column=0,pady=5,padx=50)    

            self.lth_Result_entry = ttk.Entry(self.frame_line_to_header, font="Arial 10 ")
            self.lth_Result_entry.grid(row=4,column=1,ipadx=220)





            self.line_to_header_conversion_button = ttk.Button(
                self.frame_line_to_header,
                text='Transform Lines to headers',
                command=self.convertLinetoHeaders, 
                # relief=RIDGE,
                # height= 1, 
                width=25, 
                bootstyle="success"
            ...
            self.line_to_header_conversion_button.grid(row=10, column=1,pady=20,padx=50)
    ...
    def download_line_to_header_conversion_template(self):
        try:
            # TO-DO : Change the parameters according to what you want
            parameters=['Source File name','Primary Key','Skip Cols(Completely ignore from all tests)', 'Columns to be Summed', 'Columns to be aggregated(unique values)', 'Latest Value Columns']
            df=pd.DataFrame(columns=parameters)
            pathname=os.path.join(os.path.expandvars("%userprofile%"),"Downloads")
            os.chdir(pathname)
            filename='TravisTool_LinetoHeaderConversion_Template'+'.xlsx'
...
# start_download_PostgresSQL
    ...
    def selectTestMethod(self,eventObj=np.NAN):
        ...
        elif (self.selected.get()=='Line To Header file conversion'):
            print("hi")
            self.frame_line_to_header.grid(row = 3, column = 1)
            # self.frameBatch.grid(row=3,column=1)
        # selected.get()=="Line To Header file conversion":
            # pass
    ...
    def forget_frames(self):
                ...
                self.frame_line_to_header.grid_forget()
    ...
    def select_line_to_header_Source_folder_location(self):
        global lth_Source_location_adrs
        self.lth_Source_location_adrs=fd.askdirectory(parent=self,title='Please select Source File')
        self.lth_Source_entry.delete(0,END)
        self.lth_Source_entry.insert(0,self.lth_Source_location_adrs)
        print(str(datetime.datetime.now()),": lth File Source Location selected: ",self.lth_Source_location_adrs)
    ...
    def select_lth_Result_folder_location(self):
        global lth_Result_location_adrs
        self.lth_Result_location_adrs=fd.askdirectory(parent=self,title='Please select Result directory')
        self.lth_Result_entry.delete(0,END)
        self.lth_Result_entry.insert(0,self.lth_Result_location_adrs)
        print(str(datetime.datetime.now()),": Batch Result Folder selected: ",self.lth_Result_location_adrs)
    ...
    def startPoint_lth(self):
        excelFile= fd.askopenfilename(title='Input Excel File')
        self.lthExcelPath.delete(0,END)
        self.lthExcelPath.insert(0,excelFile)
    ...
    def convertLinetoHeaders(self):
        try:
            timestampstr = datetime.datetime.now().strftime("%Y%m%d%H%M%S")  
            excelPath=self.lthExcelPath.get()
            if(excelPath == ''):
                print("THIS IS A TEST RUN, no input given to save time")
                excelPath = 'C:/Users/msinghchoudhary/Documents/Point32Health/Github Repo - Internal/Travis/TRAVIS_App-main/TRAVIS_Merged/demo/TravisTool_LinetoHeaderConversion_Template.xlsx'
                self.lthExcelPath.insert(0, excelPath)
                # excelPath = 'C:/Users/msinghchoudhary/Documents/Point32Health/Github Repo - Internal/Travis/TRAVIS_App-main/TRAVIS_Merged/demo/TravisTool_LinetoHeaderConversion_Template.xlsx'
            print(f"template Excel Path: {excelPath}")
            sourcePath = self.lth_Source_entry.get()
            if(sourcePath == ''):
                print("THIS IS A TEST RUN, no input given to save time")
                sourcePath = 'C:/Users/msinghchoudhary/Documents/Point32Health/Github Repo - Internal/Travis/TRAVIS_App-main/TRAVIS_Merged/demo/source_folder'
                self.lth_Source_entry.insert(0, sourcePath)
            
            print(f"Source Files at: {sourcePath}")
            resultPath = self.lth_Result_entry.get()
            if(resultPath == ''):
                print("THIS IS A TEST RUN, no input given to save time")
                resultPath = 'C:/Users/msinghchoudhary/Documents/Point32Health/Github Repo - Internal/Travis/TRAVIS_App-main/TRAVIS_Merged/demo/result_folder'
                self.lth_Result_entry.insert(0, resultPath)
            resultPath = resultPath+'/'+timestampstr
            print(f"Result Files at: {resultPath}")

            # Dataframe for analysis file
            df_audit = pd.DataFrame(columns=['Input file name',
                            'Row Count in Input',
                            'Unique Primary Key count Input',
                            'Unique Primary Key count Output',
                            'Duplicate Row Count-Source'
                            ])

            df_template = pd.read_excel(excelPath, header=0)
            # print(df_template.columns)
            for index, row in df_template.iterrows():
                # parameters=['Source File name','Primary Key','Skip Cols(Completely ignore from all tests)', 'Columns to be Summed', 'Columns to be aggregated(unique values)', 'Latest Value Columns']
                source_file_name = row['Source File name'].strip()
                columns_to_be_summed = [item.strip() for item in str(row['Columns to be Summed']).split(',') if item.strip() and item.lower() != 'nan']
                columns_to_be_aggregated = [item.strip() for item in str(row['Columns to be aggregated(unique values)']).split(',') if item.strip() and item.lower() != 'nan']
                latest_value_columns = [item.strip() for item in str(row['Latest Value Columns']).split(',') if item.strip() and item.lower() != 'nan']
                primary_Keys_list = [item.strip() for item in str(row['Primary Key']).strip().split(',') if item.strip() and item.lower() != 'nan']
                columns_to_be_skipped = [item.strip() for item in str(row['Skip Cols(Completely ignore from all tests)']).strip().split(',') if item.strip()  and item.lower() != 'nan']
                print(f"fileName: {source_file_name}")
                print(f"to be summed: {columns_to_be_summed}")
                print(f"to be aggregated(unique sets): {columns_to_be_aggregated}")
                print(f"Latest Value Columns: {latest_value_columns}")
                print(f"primary_Keys_list: {primary_Keys_list}")
                print(f"columns_to_be_skipped: {columns_to_be_skipped}")
                df_input_file = pd.read_excel(sourcePath+'/'+source_file_name+'.xlsx', header = 0, converters={'CBCLAIM': lambda x: str(x)})
                df_input_file = df_input_file.drop(columns = columns_to_be_skipped)
                # df_input_file['CBCLAIM'] = df_input_file['CBCLAIM'].astype(str)
                grouped_data = df_input_file.groupby(primary_Keys_list)
                # agg_functions = {'CBCLAIM': 'first'} 
                agg_functions = {}

                # date_columns = ['CAPRIMDATE','CARECDATE','CBDATE', 'CBSERVDATE','CBTHRUDATE','CBPOSTDT','CBCHECKDT']

                for column in df_input_file.columns:
                    try:
                        print(column)
                        if column in primary_Keys_list:
                            agg_functions[column] = 'first'
                            continue  # Skip the CBCLAIM column, as we already handled it
                        elif column in columns_to_be_summed:
                            # For columns with date values, keep the latest entry
                            agg_functions[column] = 'sum'
                        elif column in latest_value_columns:
                            # if(column in date_columns):
                            #     df_input_file[column] = pd.to_datetime(df_input_file[column],format='%Y%m%d')
                            #     agg_functions[column] = 'max'
                            # else:
                            agg_functions[column] = 'last'
                            
                        elif column in columns_to_be_aggregated:
                            if not pd.api.types.is_string_dtype(df_input_file[column]):
                                df_input_file[column] = df_input_file[column].fillna('').astype(str)
                            
                            # if pd.api.types.is_string_dtype(df_input_file[column]):
                                # agg_functions[column] = lambda x: list(set(x.str.strip().dropna)) 
                            print(f"{column} will be aggregated to a list of unique items")
                                # For other columns, create a list of unique values
                                #  lambda x: ';'.join(set(filter(lambda s: s.strip() != '', x.dropna()))) if isinstance(x, pd.Series) else ''
                            # agg_functions[column] = lambda x: ';'.join(set(filter(lambda s: s.strip() != '', x.dropna()))) if isinstance(x, pd.Series) else ''
                            agg_functions[column] = lambda x: ",".join(list(set(filter(None,x.str.strip().dropna()))) if isinstance(x, pd.Series) else '')
                            # else:
                                # agg_functions[column] = lambda x: list(set(x)) 
                                # print(f"{column} will be aggregated to a list of unique items")
                                # For other columns, create a list of unique values
                                # agg_functions[column] = lambda x: list(set(x.dropna())) if isinstance(x, pd.Series) else print("someerror")
                            

                        else:
                            print(f"{column} has no rule, so everything will be put in a list")
                            if pd.api.types.is_string_dtype(df_input_file[column]):
                                agg_functions[column] = lambda x: list(filter(None,x.str.strip().dropna())) if isinstance(x, pd.Series)  else ''
                            else:
                                agg_functions[column] = lambda x: list(x.dropna()) if isinstance(x, pd.Series)  else ''
                    except Exception as e:
                        print(f"Error Occured in {column}: {str(e)}")

                # Apply the aggregation functions to group the data
                df_grouped = grouped_data.agg(agg_functions)
                df_grouped.fillna('')
                # Now, df_grouped contains the desired aggregation for each unique "claimnumber"
                
                # df_summary to get count of total rows of each unique claim/ unique primary key
                df_summary = df_input_file.groupby(primary_Keys_list[0]).size().reset_index(name='Count')


                # dict to add data into df_audit
                audit_data_dict = {'Input file name': source_file_name,
                                    'Row Count in Input': df_input_file.shape[0],
                                    'Unique Primary Key count Input': df_input_file[primary_Keys_list[0]].nunique(),
                                    'Unique Primary Key count Output': df_grouped[primary_Keys_list[0]].nunique(),
                                    'Duplicate Row Count-Source': df_input_file[df_input_file.duplicated(subset=['CBCLAIM', 'CBLINE', 'CBSUBLINE'], keep=False)].shape[0]
                                     }
                
                df_temp = pd.DataFrame([audit_data_dict])   
                df_audit = pd.concat([df_audit,df_temp], ignore_index=True)
                del df_temp
                
                # Now, df_audit contains the analysis details for each input file

                # Save the resulting DataFrame to a new Excel file if needed
                output_excel_file = resultPath+'/'+'output_'+source_file_name+'.xlsx'  # Replace with your desired output file path
                dir_name = os.path.dirname(output_excel_file)
                ...
                df_grouped.to_excel(writer, sheet_name = 'Header File',index=False)
                df_summary.to_excel(writer, sheet_name = 'Summary',index=False)
                writer.close()

                # del df_audit

                messagebox.showinfo("Processing Complete",f"Check Result at: {output_excel_file.rsplit('/',1)[0]}")
            # messagebox.showinfo("Processing Complete","All specified testing & consolidation of reports is done. Check Result folder")
        

            # You can also display the resulting DataFrame
                print(df_grouped)

            # Save the resulting Analysis DataFrame to an excel file
            output_analysis_file = resultPath+'/'+timestampstr+' audit.xlsx'
            ana_dir_name = os.path.dirname(output_analysis_file)
            if(os.path.exists(ana_dir_name)):
                print(f"{ana_dir_name} exists")
            else:
                os.mkdir(ana_dir_name)
            df_audit.to_excel(output_analysis_file, index=False)

        except Exception as e:
            messagebox.showinfo("Processing Failed",str(e))
