...
class master(tkb.Window):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.__travis_deloitte_full_image = None 
        ...
        self.run_dictionary = {} 
        self.__key_data = {}
        self.__running_option = []
        
        # directory locations 
        self.__static_directory = os.path.join(os.path.dirname(os.path.abspath(__file__)), master.STATIC_FOLDER_NAME)  
        self.__template_directory = os.path.join(os.path.dirname(os.path.abspath(__file__)), master.TEMPLATE_FOLDER_NAME)
        self.__config_directory = os.path.join(os.path.dirname(os.path.abspath(__file__)), master.CONFIG_FILE_NAME) 
        ...
        self.__config_data = friday_reusable.get_config_data(self.__config_directory)
        ...
        self.__get_valid_token()        

        # set up user workspace 
        self.__mypath, self.__run_date = self.__setup_user_workspace()
    ...
    def __get_valid_token(self) -> None:
        
        try:
            token_file = open(os.path.join(self.__static_directory, "token.dat"), "r")
            self.__token_data = token_file.read()
            encrypted_string = bytes(self.__token_data, "utf-8")
            key = self.__gui_config.get("token_key", None)
            travis_title = self.__gui_config.get("travis_title", None)

            # if key not present - end the program abruptly 
            if key is None:
                messagebox.showerror(travis_title, "No Token. Please contact TRAVIS Team")
                sys.exit()

            key_bytes = bytes(key, "utf-8")

            # create Fernet encrypted data
            fernet_object = Fernet(key_bytes)

            # decrypt the read string 
            decrypted_string = fernet_object.decrypt(encrypted_string)

            # get start and valid days 
            start_date, valid_days = decrypted_string.decode().split("#")

            # get current date and time 
            current_date = time.time()  
            start_date_formatted = datetime.datetime.fromtimestamp(float(start_date))
            current_date_formatted = datetime.datetime.fromtimestamp(current_date)

            # get date difference 
            diff = current_date_formatted - start_date_formatted
            hours_used = diff.total_seconds()/3600
            days_used = hours_used/24

            # throw error if future key 
            if float(current_date) < float(start_date):
                messagebox.showerror(travis_title, "Invalid Token. Given token can be used for Future date only. Please contact TRAVIS Team")
                sys.exit()

            if days_used > float(valid_days): 
                messagebox.showerror(travis_title, "Expired Token. Token has expired, please contact TRAVIS Team")
                sys.exit()

        except Exception as e:
            print (e)
            messagebox.showerror("TRAVIS.", "Fatal Error")
            sys.exit()



    def __setup_user_workspace(self) -> str: 
        """ set up user workspace """

        workspace_directory = self.__gui_config.get("workspace_directory", None)
    ...
    def __set_up_log_handle(self) -> None:
        ...
        logging.basicConfig(filename=os.path.join(self.__mypath, master.LOG_FILE_NAME), 
                            filemode='w',
                            level=log_level, 
    ...
    def __get_gui_configurations(self) -> None: 
        ...
        self.__travis_title = self.__gui_config.get("travis_title", "TRAVIS.")
    ...
    def __setup_travis_gui(self) -> 'None': 
        ...
        self.title(self.__travis_title)
    ...
    def __populate_header_section(self) -> None:
        ...
        # populate label, entry and show widgets for workspace 
        self.__populate_workspace_data_header()
    ...
    def __populate_workspace_data_header(self) -> None:
    ...
    def __populate_application_name_selection(self) -> None:
        ...
        application_label                   = tkb.Label(self.selection_section, 
                                                        text='Application: ')
        application_label.grid(row=0, column=2, sticky="e", padx=(10, 10), pady=(10, 10))
        ...
        self.application_entry.grid(row=0, column=3, sticky="w", padx=(10, 10), pady=(10, 10))    
    ...
    def __populate_environment_name_selection(self) -> None:
        ...
        environment_label                   = tkb.Label(self.selection_section, 
                                                        text='Environment: ')
        environment_label.grid(row=0, column=4, sticky="e", padx=(10, 10), pady=(10, 10))
        ...
        self.environment_entry.grid(row=0, column=5, sticky="w", padx=(10, 10), pady=(10, 10))
    ...
    def __evaluate_first_option(self, selection_event) -> None:
        ...
        if "please select" not in self.first_option_selected.lower():
            ...
            for tab_name in list(temp_config):
                ...
                self.execution_notebook.add(execution_frame, text=tab_name)
    ...
    def __evaluate_second_option(self, second_option_selected, temp_config, execution_frame) -> None:
        ...
        execution_canvas.pack(side=LEFT, fill=BOTH, expand=1)
        execution_canvas.pack_propagate(False)

        # create scrollbar on exection frame 
        execution_scrollbar        = tkb.Scrollbar(execution_frame, 
                                                   orient=VERTICAL, 
                                                   bootstyle="round")
        execution_scrollbar.pack(side=RIGHT, fill=Y)            
        
        # configure the scroll bar
        execution_scrollbar.config(command=execution_canvas.yview)              

        # configure the canvas
        execution_canvas.configure(yscrollcommand=execution_scrollbar.set)

        # bind the canvas to scrolling event 
        execution_canvas.bind("<Configure>", lambda e:execution_canvas.configure(scrollregion=execution_canvas.bbox("all")))
        execution_canvas.bind("<MouseWheel>", lambda e: execution_canvas.yview_scroll(int(-1*(e.delta/120)), "units"))

        # create another frame inside the canvas 
        execution_section = tkb.Frame(execution_canvas)
        execution_canvas.create_window((0,0), window=execution_section, anchor="nw", width=self.__screen_width-execution_scrollbar.winfo_width()-50)
        ...
        execution_canvas.configure(scrollregion=execution_canvas.bbox(ALL))
    ...
    def __process_request(self):
        ...
        configuration, application_name, environment_name = self.__get_app_env_name(first_option_selected, second_option_selected)

        message = ""
        message = self.__validate_app_env_name(application_name, environment_name, second_option_selected)
        if message != "":
        ...
        try:
            if first_option_selected in ['COMPARE_FILE_METADATA', ]:
